 ...
 * Compatible with both Deno (Edge Functions) and Node.js (tests)
 */
...
export class MetricsCollector {
  ...
  // Flush metrics to database (mock implementation for tests)
  async flush(): Promise<void> {
    ...
    try {
      // In a real Edge Function, this would use Supabase client
      // For now, just log the metrics
      console.log(`Flushed ${metricsToFlush.length} structured metrics to database`);
    } catch (error) {
...
export function setupMetricsCleanup() {
  ...
  if (typeof Deno !== 'undefined') {
    ...
    try {
      ...
      // Ignore if signal listeners are not available
    }
  }
