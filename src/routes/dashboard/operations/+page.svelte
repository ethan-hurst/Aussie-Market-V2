<script lang="ts">
  ...
  // Auto-refresh data every 15 seconds for operations
  ...
  async function refreshData() {
    isLoading = true;
    try {
      const [operationalResponse, performanceResponse, eventsResponse] = await Promise.all([
      ...
      const [operationalResult, performanceResult, eventsResult] = await Promise.all([
  ...
  $: activeConnections = operationalData.operational
    .filter((m: any) => m.metric_name === 'active_connections')
    .slice(-1)[0]?.metric_value || 0;
...
<svelte:head>
  <title>Operations Dashboard - Aussie Market</title>
</svelte:head>
...
<div class="space-y-6">
  ...
  <div class="flex justify-between items-center">
    <div>
      ...
      <p class="text-gray-600 mt-1">Real-time system performance and operational metrics</p>
    </div>
  ...
  {#if error}
    <div class="rounded-md bg-red-50 p-4">
      <div class="flex">
        ...
        <div class="ml-3">
          <h3 class="text-sm font-medium text-red-800">Error loading operations data</h3>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <div class="bg-white overflow-hidden shadow rounded-lg">
      <div class="p-5">
        <div class="flex items-center">
          <div class="flex-shrink-0">
            <svg class="h-6 w-6 {errorRate < 1 ? 'text-green-600' : errorRate < 5 ? 'text-yellow-600' : 'text-red-600'}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="ml-5 w-0 flex-1">
            <dl>
              ...
              <dd class="text-lg font-medium {errorRate < 1 ? 'text-green-600' : errorRate < 5 ? 'text-yellow-600' : 'text-red-600'}">{errorRate.toFixed(2)}%</dd>
            </dl>
          </div>
        </div>
      </div>
        ...
        <div class="flex items-center">
          ...
          <div class="ml-5 w-0 flex-1">
            <dl>
              ...
              <dd class="text-lg font-medium text-gray-900">{activeConnections.toLocaleString()}</dd>
            </dl>
          </div>
        </div>
      </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <div class="flex items-center">
        <div class="text-3xl font-bold {avgResponseTime < 200 ? 'text-green-600' : avgResponseTime < 500 ? 'text-yellow-600' : 'text-red-600'}">{avgResponseTime.toFixed(0)}ms</div>
        <div class="ml-4">
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <div class="mt-1 bg-gray-200 rounded-full h-2">
            <div class="bg-purple-600 h-2 rounded-full transition-all duration-300" style="width: {memoryUsage}%"></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          {#each eventsData.slice(0, 20) as event}
