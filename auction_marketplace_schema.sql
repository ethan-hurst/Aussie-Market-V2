...
-- Safe script that checks for existing objects before creating them
...
DROP POLICY IF EXISTS "seller_rw" ON public.listings;
CREATE POLICY "seller_rw" ON public.listings FOR ALL
  USING (seller_id = auth.uid()) WITH CHECK (seller_id = auth.uid());

DROP POLICY IF EXISTS "public_read_live" ON public.listings;
...
DROP POLICY IF EXISTS "listing_photos_seller_rw" ON public.listing_photos;
...
CREATE INDEX IF NOT EXISTS bids_auction_id_placed_at_idx ON public.bids (auction_id, placed_at);
...
-- Bids RLS policies
DROP POLICY IF EXISTS "bidder_own_read" ON public.bids;
CREATE POLICY "bidder_own_read" ON public.bids FOR SELECT USING (bidder_id = auth.uid());

DROP POLICY IF EXISTS "bidder_create" ON public.bids;
...
DROP POLICY IF EXISTS "ledger_admin_only" ON public.ledger_entries;
...
DROP POLICY IF EXISTS "pickup_party_read" ON public.pickups;
CREATE POLICY "pickup_party_read" ON public.pickups FOR SELECT
  USING (order_id IN (SELECT id FROM public.orders WHERE buyer_id = auth.uid() OR seller_id = auth.uid()));

DROP POLICY IF EXISTS "shipment_party_read" ON public.shipments;
...
DROP POLICY IF EXISTS "threads_party_read" ON public.message_threads;
...
DROP POLICY IF EXISTS "metrics_admin_only" ON public.metrics;
CREATE POLICY "metrics_admin_only" ON public.metrics FOR ALL
  USING (auth.jwt() ->> 'role' = 'admin');

-- Create indexes for performance (only if they don't exist)
CREATE INDEX IF NOT EXISTS listings_seller_id_status_idx ON public.listings (seller_id, status);
CREATE INDEX IF NOT EXISTS listings_category_id_status_idx ON public.listings (category_id, status);
CREATE INDEX IF NOT EXISTS listings_end_at_idx ON public.listings (end_at) WHERE status = 'live';
CREATE INDEX IF NOT EXISTS auctions_status_idx ON public.auctions (status);
CREATE INDEX IF NOT EXISTS orders_buyer_id_state_idx ON public.orders (buyer_id, state);
CREATE INDEX IF NOT EXISTS orders_seller_id_state_idx ON public.orders (seller_id, state);
CREATE INDEX IF NOT EXISTS orders_created_at_idx ON public.orders (created_at);
CREATE INDEX IF NOT EXISTS ledger_entries_order_id_entry_type_idx ON public.ledger_entries (order_id, entry_type);
CREATE INDEX IF NOT EXISTS disputes_state_idx ON public.disputes (state);
...
CREATE INDEX IF NOT EXISTS metrics_metric_name_metric_date_idx ON public.metrics (metric_name, metric_date);
