-- Database State Inspection Script (Fixed Version)
...
-- 5. Check Row Level Security (RLS) status - Fixed version
SELECT 
    schemaname,
    tablename,
    CASE 
...
-- 13. Summary of what should be created vs what exists
SELECT 
    'EXPECTED TABLES SUMMARY' as summary_type,
    COUNT(*) as count,
    'Should have 15 auction marketplace tables' as description
FROM (
    SELECT 'users' as table_name
    UNION ALL SELECT 'listings'
    UNION ALL SELECT 'listing_photos'
    UNION ALL SELECT 'auctions'
    UNION ALL SELECT 'bids'
    UNION ALL SELECT 'orders'
    UNION ALL SELECT 'payments'
    UNION ALL SELECT 'ledger_entries'
